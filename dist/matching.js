...
export function rgbaMatToTemplate(cv, rgba, metadata) {
...
export function scaleTemplate(cv, template, scale) {
...
function deleteScaled(template) {
    if (!template.owned)
        return;
    template.rgb.delete();
    template.mask.delete();
}
function bestMatch(cv, image, template) {
    if (template.width > image.cols || template.height > image.rows)
        return null;
    ...
    const search = image.roi(new cv.Rect(searchX, searchY, searchRight - searchX, searchBottom - searchY));
    let result;
    let noMask;
    try {
        result = new cv.Mat();
...
/** @internal */
export function makeBorderMask(cv, alphaMask, borderWidth) {
...
function assignGridCoordinates(slots) {
    const sorted = [...slots].sort((left, right) => left.y - right.y || left.x - right.x);
    const rows = [];
    for (const slot of sorted) {
        const centerY = slot.y + slot.height / 2;
        let row = rows.find((candidateRow) => {
            const meanCenter = candidateRow.reduce((sum, candidate) => sum + candidate.y + candidate.height / 2, 0) /
                candidateRow.length;
...
export function locateSlots(cv, imageRgb, templates, options) {
    const candidates = [];
    for (const template of templates) {
        for (const scale of options.scales) {
            ...
            try {
                ...
                const maxima = collectLocalMaxima(result, options.threshold, scaled.width, scaled.height, scale, template.metadata.id)
    ...
    const selected = [];
    for (const candidate of candidates.sort((left, right) => right.localizationConfidence - left.localizationConfidence)) {
        if (selected.some((slot) => intersectionOverUnion(slot, candidate) >= 0.45))
            continue;
        selected.push(candidate);
        if (selected.length >= options.maxSlots)
            break;
    }
    return assignGridCoordinates(selected);
}
function toMatchAlternative(template, match) {
...
export function matchItemsInSlot(cv, imageRgb, slot, templates, nativeSlotSize, alternativeCount) {
    ...
    for (const match of matches) {
        ...
        if (!previous || match.confidence > previous.confidence)
            bestByName.set(match.name, match);
    }
