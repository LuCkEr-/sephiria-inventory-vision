...
import { cosineDescriptorSimilarity, extractEdgeShapeFeatures, extractOrbDescriptors, extractPerceptualHash, extractPixelVector, orbSimilarity, perceptualHashSimilarity, } from "./method-features.js";
export const CLASSICAL_METHODS = [
    "vision-features",
    "perceptual-hash",
    "edge-shape",
    "orb",
...
export function prepareMethodCell(cell) {
...
function score(method, query, reference) {
    switch (method) {
...
function bestByLabel(candidates) {
...
export function classifyPreparedCell(query, references, method, alternatives = 3) {
    ...
    if (method !== "hybrid-retrieval") {
        return bestByLabel(references.map((reference) => ({
    ...
    const shortlist = bestByLabel(references.map((reference) => ({
        label: reference.label,
...
export function classifyMethodCell(query, references, method, alternatives = 3) {
    return classifyPreparedCell(prepareMethodCell(query), references, method, alternatives);
}
//# sourceMappingURL=method-lab.js.map