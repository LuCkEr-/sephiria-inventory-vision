...
function extractPixelSignature(pixels, width, channels) {
    const values = [];
    for (let outputIndex = 0; outputIndex < PIXEL_FEATURE_SIZE; outputIndex += 1) {
        ...
        for (let dy = 0; dy < 2; dy += 1) {
            for (let dx = 0; dx < 2; dx += 1) {
                const x = cellX * 2 + dx;
                const y = cellY * 2 + dy;
                value += pixels[(y * width + x) * channels + channel];
            }
        }
...
export function cosineSimilarity(left, right) {
    if (left.length !== right.length) {
        throw new Error(`Cosine similarity requires equal vector lengths, received ${left.length} and ${right.length}`);
    }
    let dot = 0;
...
export function normalizedDotProduct(left, right) {
    let score = 0;
    for (let index = 0; index < left.length; index += 1) {
...
function validateCellShape(pixels, width, height, channels) {
    ...
    if (pixels.length < expectedValues) {
        throw new Error(`Vision features require ${expectedValues} pixel values, received ${pixels.length}`);
    }
}
function byteValue(value, pixelIndex) {
...
function extractChannels(pixels, width, height, channels) {
...
function appendColorFeatures(features, channels, width, height) {
    const colorCells = 4;
        ...
        for (let cellX = 0; cellX < colorCells; cellX += 1) {
            ...
            for (let y = cellY * cellHeight; y < (cellY + 1) * cellHeight; y += 1) {
                for (let x = cellX * cellWidth; x < (cellX + 1) * cellWidth; x += 1) {
                    ...
                    red += channels.red[index];
                    green += channels.green[index];
                    blue += channels.blue[index];
                    count += 1;
                }
            }
            const sum = red + green + blue || 1;
            features.push((red / sum) * 0.7, (green / sum) * 0.7, (blue / sum) * 0.7, ((red + green + blue) / count / 3) * 0.25);
        }
    }
}
function appendHogFeatures(features, gray, width, height) {
    const cells = 4;
    const bins = 9;
    ...
    for (let y = 1; y < height - 1; y += 1) {
        for (let x = 1; x < width - 1; x += 1) {
            ...
            const histogram = histograms[cellY * cells + cellX];
            histogram[bin] = histogram[bin] + magnitude;
        }
    }
    for (const histogram of histograms) {
        for (const value of normalize([...histogram]))
            features.push(value * 1.5);
    }
}
function appendLuminanceFeatures(features, gray, grayMean, grayStd, width) {
    const cells = 8;
    const cellSize = 4;
 ...
 */
export function extractVisionFeatures(pixels, width, height, channels = 3) {
...
//# sourceMappingURL=vision-features.js.map