import type * as TensorFlowNamespace from "@tensorflow/tfjs";
...
type TensorFlow = typeof TensorFlowNamespace;
type SymbolicTensor = TensorFlowNamespace.SymbolicTensor;
type Tensor = TensorFlowNamespace.Tensor;
type Tensor2D = TensorFlowNamespace.Tensor2D;
type Tensor4D = TensorFlowNamespace.Tensor4D;
type Scalar = TensorFlowNamespace.Scalar;
type LayersModel = TensorFlowNamespace.LayersModel;
type Optimizer = TensorFlowNamespace.Optimizer;

const tensorflowLoader = new RetryableLoader<TensorFlow>(
  () => import("@tensorflow/tfjs"),
  "Neural lab methods require the optional @tensorflow/tfjs peer dependency",
);

function getTensorFlow(): Promise<TensorFlow> {
  return tensorflowLoader.get();
}

function validatePixels(pixels: ArrayLike<number>, context: string): void {
  const expectedLength = 32 * 32 * 3;
  if (pixels.length !== expectedLength) {
    throw new Error(
      `${context} must contain 32x32 RGB pixels exactly, received ${pixels.length} values`,
    );
  }
...
function asPixels(samples: readonly NeuralCellSample[]): Float32Array {
...
function outputTensor(value: Tensor | Tensor[]): Tensor {
  if (Array.isArray(value)) {
    const first = value[0];
    if (!first) throw new Error("Model returned no tensors");
    return first;
  }
  return value;
}

async function prepareTensorFlow(): Promise<TensorFlow> {
...
function createTinyCnnModels(
  tf: TensorFlow,
  labelCount: number,
  seed: number,
  learningRate: number,
): { model: LayersModel; embedder: LayersModel } {
  ...
  value = tf.layers.maxPooling2d({ poolSize: 2 }).apply(value) as SymbolicTensor;
  value = tf.layers
    .conv2d({
      filters: 16,
      kernelSize: 3,
  ...
  const output = tf.layers
    ...
    .apply(embedding) as SymbolicTensor;
...
async function fitTinyCnn(
  tf: TensorFlow,
  model: LayersModel,
  samples: readonly NeuralCellSample[],
  labelIndex: ReadonlyMap<string, number>,
  options: TrainOptions,
): Promise<void> {
  const xs = tf.tensor4d(asPixels(samples), [samples.length, 32, 32, 3]);
  const ys = tf.tensor1d(
    samples.map((sample) => labelIndex.get(sample.label) ?? 0),
    "float32",
  );
  try {
...
function cnnInputTensor(tf: TensorFlow, pixels: ArrayLike<number>): Tensor4D {
...
function bestProbabilityIndex(probabilities: ArrayLike<number>, count: number, offset = 0): number {
  let bestIndex = 0;
...
function createTinyCnnRuntime(
  tf: TensorFlow,
  model: LayersModel,
  embedder: LayersModel,
  labels: string[],
): TrainedTinyCnn {
  let disposed = false;
  const assertActive = (): void => {
    if (disposed) throw new Error("Tiny CNN has been disposed");
  };
  const embed = (pixels: ArrayLike<number>): number[] =>
    tf.tidy(() => {
...
function predictCnnBatch(
  tf: TensorFlow,
  model: LayersModel,
  embedder: LayersModel,
  labels: readonly string[],
  cells: readonly ArrayLike<number>[],
): NeuralPrediction[] {
  return tf.tidy(() => {
    ...
    return cells.map((_, cellIndex) => {
      ...
      return {
        ...
        embedding: Array.from(
          embeddings.slice(cellIndex * embeddingSize, (cellIndex + 1) * embeddingSize),
        ),
      };
    });
  });
...
export async function trainTinyCnn(
  samples: readonly NeuralCellSample[],
  options: TrainOptions = {},
): Promise<TrainedTinyCnn> {
  if (samples.length === 0) throw new Error("Cannot train a CNN without samples");
  ...
  const { model, embedder } = createTinyCnnModels(
    tf,
    labels.length,
    seed,
    options.learningRate ?? 0.004,
  );
  try {
...
function normalizedTensor(tf: TensorFlow, tensor: Tensor2D): Tensor2D {
  return tf.div(tensor, tf.maximum(tf.norm(tensor, 2, 1, true), tf.scalar(1e-8)));
}

interface SiamesePairBatch {
  left: number[];
  right: number[];
  targets: number[];
}

function validateSiameseSamples(samples: readonly FeatureCellSample[]): number {
  samples.forEach((sample) => {
...
function createSiameseTower(tf: TensorFlow, featureSize: number, seed: number): LayersModel {
...
function groupSamplesByLabel(samples: readonly FeatureCellSample[]): Map<string, number[]> {
...
function differentLabel(labels: readonly string[], excluded: string, random: () => number): string {
  let label = labels[Math.floor(random() * labels.length)] as string;
  while (label === excluded && labels.length > 1) {
    label = labels[Math.floor(random() * labels.length)] as string;
  }
  return label;
}

function randomBucketIndex(
  byLabel: ReadonlyMap<string, readonly number[]>,
  label: string,
  random: () => number,
): number {
  const bucket = byLabel.get(label) as readonly number[];
  return bucket[Math.floor(random() * bucket.length)] as number;
}

function createSiamesePairs(
  samples: readonly FeatureCellSample[],
  byLabel: ReadonlyMap<string, readonly number[]>,
  labels: readonly string[],
  pairCount: number,
  random: () => number,
): SiamesePairBatch {
  const batch: SiamesePairBatch = { left: [], right: [], targets: [] };
  for (let pair = 0; pair < pairCount; pair += 1) {
    const same = pair % 2 === 0;
    const leftIndex = Math.floor(random() * samples.length);
    const leftSample = samples[leftIndex] as FeatureCellSample;
    ...
    const rightSample = samples[rightIndex] as FeatureCellSample;
...
function contrastiveLoss(
  ...
  tower: LayersModel,
  leftTensor: Tensor2D,
  rightTensor: Tensor2D,
  targetTensor: Tensor,
): Scalar {
  const leftEmbedding = normalizedTensor(
    tf,
    outputTensor(tower.apply(leftTensor, { training: true }) as Tensor) as Tensor2D,
  );
  const rightEmbedding = normalizedTensor(
    tf,
    outputTensor(tower.apply(rightTensor, { training: true }) as Tensor) as Tensor2D,
  );
  const distances = tf.sqrt(
    tf.maximum(tf.sum(tf.square(tf.sub(leftEmbedding, rightEmbedding)), 1), tf.scalar(1e-8)),
  );
  const positiveLoss = tf.mul(targetTensor, tf.square(distances));
  const negativeLoss = tf.mul(
    tf.sub(1, targetTensor),
    tf.square(tf.maximum(tf.sub(1, distances), 0)),
  );
  return tf.mean(tf.add(positiveLoss, negativeLoss));
}

function trainSiameseBatch(
  tf: TensorFlow,
  tower: LayersModel,
  optimizer: Optimizer,
  batch: SiamesePairBatch,
  pairCount: number,
  featureSize: number,
): void {
  ...
  try {
    optimizer.minimize(
      () => tf.tidy(() => contrastiveLoss(tf, tower, leftTensor, rightTensor, targetTensor)),
      false,
    );
  } finally {
    leftTensor.dispose();
...
function createSiameseRuntime(
  ...
  return {
    embed(features) {
      if (disposed) throw new Error("Siamese embedding has been disposed");
      ...
      return tf.tidy(() => {
        ...
        const embedded = normalizedTensor(tf, outputTensor(tower.predict(tensor)) as Tensor2D);
...
export async function trainSiameseEmbedding(
  samples: readonly FeatureCellSample[],
  options: TrainOptions = {},
): Promise<TrainedSiameseEmbedding> {
  if (samples.length < 2) throw new Error("Cannot train a Siamese model without sample pairs");
  validateTrainOptions(options);
